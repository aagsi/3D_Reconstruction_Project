...
def capture_point_cloud_from_camera(pipeline, pc):
    ...
    # Use GPU to downsample the point cloud, provide dtype and device explicitly
    device = o3d.core.Device("CUDA:0")
    pcd_frame_cuda = o3d.t.geometry.PointCloud.from_legacy(pcd_frame, o3d.core.Dtype.Float32, device)
    pcd_frame_cuda = pcd_frame_cuda.voxel_down_sample(voxel_size=0.01)

    return pcd_frame_cuda.to_legacy()

# Align point clouds using legacy ICP
def align_point_clouds(source, target):
    threshold = 0.02  # Adjust for registration
    trans_init = np.eye(4)  # Initial transformation

    # Perform ICP using the legacy API (on CPU)
...
# Load the point cloud from the file (legacy format)
pcd = o3d.io.read_point_cloud("captured_data_on_the_fly.ply")

# Convert the legacy point cloud to tensor-based (for CUDA operations)
device = o3d.core.Device("CUDA:0")
pcd_t = o3d.t.geometry.PointCloud.from_legacy(pcd, o3d.core.Dtype.Float32, device)

# Step 5: Downsample the point cloud using CUDA
pcd_down = pcd_t.voxel_down_sample(voxel_size=0.0025)

# Convert back to legacy if needed for further legacy operations
...
# Step 7: Estimate normals using CUDA
# Convert the inlier point cloud from legacy to tensor-based for CUDA operations
pcd_inlier_cuda = o3d.t.geometry.PointCloud.from_legacy(pcd_inlier, o3d.core.Dtype.Float32, device)

# Estimate normals on the tensor-based PointCloud using CUDA
pcd_inlier_cuda.estimate_normals(max_nn=50, radius=0.05)

# Orient normals consistently
pcd_inlier_cuda.orient_normals_consistent_tangent_plane(100)

# Convert the tensor-based PointCloud back to legacy
pcd_inlier = pcd_inlier_cuda.to_legacy()

# Step 8: Reconstruct the 3D mesh using Poisson surface reconstruction
