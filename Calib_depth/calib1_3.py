...
def stereoCalibrateCamera(camera_c1, camera_c2, camera_name, chessboard_box_size=47, 
                          chessboard_grid_size=(7,7), number_of_frames=100):
    """
    ...
    - number_of_frames: Number of calibration frames to capture (default: 100).
    """
    ...
    stereocalibration_flags = cv2.CALIB_FIX_INTRINSIC | cv2.CALIB_ZERO_TANGENT_DIST | cv2.CALIB_RATIONAL_MODEL
    ...
    print("Instructions:")
    print("- Ensure the chessboard is visible in both cameras.")
    print("- Move the chessboard around to different positions and angles.")
    print("- Vary the distance and orientation of the chessboard.\n")
    ...
    for idx, (image1, image2) in enumerate(zip(img_list_c1, img_list_c2), start=1):
        ...
        ret1, corners1 = cv2.findChessboardCorners(
            gray1, CHECKERBOARD, None, cv2.CALIB_CB_ADAPTIVE_THRESH + cv2.CALIB_CB_NORMALIZE_IMAGE)
        ret2, corners2 = cv2.findChessboardCorners(
            gray2, CHECKERBOARD, None, cv2.CALIB_CB_ADAPTIVE_THRESH + cv2.CALIB_CB_NORMALIZE_IMAGE)
    ...
    if valid_pairs < 10:
        print("Insufficient valid image pairs for calibration. Need at least 10 valid pairs.")
        return
    else:
        print(f"Total valid pairs for calibration: {valid_pairs}")
...
def main():
    ...
    try:
        ...
        baseline = np.linalg.norm(T)
        print(f"Baseline between cameras: {baseline} mm")
